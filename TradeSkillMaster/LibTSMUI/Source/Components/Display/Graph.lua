...
function Graph:__init()
	...
	-- Redraw the graph when its size changes (e.g. when the application frame is resized) so the
	-- plotted line and axes are recomputed to fit the new plot size instead of overflowing the frame.
	frame:TSMSetScript("OnSizeChanged", self:__closure("_HandlePlotSizeChanged"))
...
function Graph:Draw()
	...
	-- 3.3.5: если точек для отрисовки нет (вырожденный/пустой диапазон zoom, например сразу после
	-- zoom-drag или при очень редких данных), прерываемся до сравнения цвета ниже -
	-- иначе self._yLookup[…[1]] <= self._yLookup[…[#]] сравнивает nil с nil и кидает Lua-ошибку,
	-- которая обрывает Draw() и последующий PopulateDetails → график и цифры Dashboard пустеют.
	if #self._xValuesFiltered == 0 then
		for _, slice in ipairs(self._slices) do
			slice:Hide()
			slice.fillTop:Hide()
			slice.fillBar:Hide()
		end
...
function Graph.__private:_ComputeData(plotWidth, plotHeight)
	...
	-- 3.3.5: при ресайзе окна / крошечной высоте plot (maxNumSteps == 0 → деление на ноль в
	-- GraphYStepFunc) или при вырожденном диапазоне RANGE может вернуть NaN или _yMin == _yMax.
	-- Раньше проверялся только _yMax, поэтому NaN из _yMin (или нулевой знаменатель при равных
	-- границах) утекал в Math.Scale ниже и давал NaN-координаты → «зубцы»/артефакты на кривой.
	-- В этих случаях прерываемся — на следующем кадре (после OnSizeChanged) перерисуемся корректно.
	if Math.IsNan(self._yMax) or Math.IsNan(self._yMin) or self._yMin == self._yMax then
...
function Graph.__private:_HandlePlotSizeChanged()
	-- Defer the redraw by one frame so the plot's anchor-derived size has been realized before we
	-- recompute the plotted data. The base frame's OnUpdate is otherwise unused, so we reuse it as a
	-- one-shot timer (the hover OnUpdate lives on frame.plot, so there's no conflict).
	local frame = self:_GetBaseFrame()
	frame:TSMSetScript("OnUpdate", self:__closure("_HandleResizeRedraw"))
end

function Graph.__private:_HandleResizeRedraw()
	local frame = self:_GetBaseFrame()
	frame:TSMSetScript("OnUpdate", nil)
	-- Only redraw once the graph has been acquired and fully configured with a range and value
	-- function; otherwise Draw()/_ComputeData would error on nil zoom bounds.
	if not self._acquired or not self._zoomStart or not self._zoomEnd or not self._yValueFunc then
		return
	end
	self:Draw()
end
