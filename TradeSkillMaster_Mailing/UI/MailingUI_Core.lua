...
local MailService = TSM.LibTSMService:Include("Mail")
local private = {
	settings = nil,
	topLevelPages = {},
	frame = nil,
...
function private.FSMCreate()
	...
	private.fsm = FSM.New("MAILING_UI")
		...
		:AddState(FSM.NewState("ST_FRAME_OPEN")
			...
			:AddEvent("EV_FRAME_HIDE", function(context)
				CancelEmote()
				-- 3.3.5: TSM не открывал баги в EV_MAIL_SHOW (OpenAllBags под guard not IsRetail),
				-- значит и закрывать не должен — это ломает state любого Bags-аддона
				-- если юзер открыл баги вручную или другой аддон управляет ими.
				if ClientInfo.IsRetail() then
					CloseAllBags()
				end
				CloseMail()
			...
			:AddEvent("EV_MAIL_SHOW", function(context)
				...
				CheckInbox()
				-- 3.3.5: MAIL_INBOX_UPDATE может прилететь раньше чем сервер
				-- доставит inbox payload — первая открытие даёт пустой scan.
				-- Принудительный rescan через 1s страхует от этого race.
				if not ClientInfo.IsRetail() then
					MailService.ForceRescan(1.0)
				end
